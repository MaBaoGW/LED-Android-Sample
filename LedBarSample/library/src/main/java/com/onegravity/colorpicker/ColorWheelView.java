...
public class ColorWheelView extends View {
     ...
     * </p>
     */
    //private static final int[] COLORS = {0xFF000000, 0xFFFFFFFF};
    ...
    private void setColorRGB(int color) {
    ...
    private float colorToAngle(int color) {
        ...
        Log.d(TAG, "colorToAngle " + -colors[0]);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mUserIsMovingPointer) {
                    ...
                    Log.d(TAG, "ACTION_MOVE mAngle = " + mAngle);
                    mPointerColor.setColor(calculateColor(mAngle));

                    setNewCenterColor(mCenterNewColor = calculateColor(mAngle));

                    String[] colorComponents = Util.convertToARGB(calculateColor(mAngle));
                    mExactViewR.setText(colorComponents[1]);
                    mExactViewG.setText(colorComponents[2]);
                    mExactViewB.setText(colorComponents[3]);
    ...
    public void addRedText(EditText edittext){
        mExactViewR = edittext;
        mExactViewR.addTextChangedListener(mExactTextWatcher);
    }

    public void addGreenText(EditText edittext){
        mExactViewG = edittext;
        mExactViewG.addTextChangedListener(mExactTextWatcher);
    }

    public void addBlueText(EditText edittext){
        mExactViewB = edittext;
    ...
    private TextWatcher mExactTextWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            ...
            //Log.d(TAG, "mExactTextWatcher " + s);

            try {
                int color = Util.convertToColorInt(
                        "ff", mExactViewR.getText().toString(),
                        mExactViewG.getText().toString(), mExactViewB.getText().toString(),
                        false);
                Log.d(TAG, "TextWatcher color = " + color);

                setNewCenterColor(mCenterNewColor = color);
                //mAngle = colorToAngle(color);
